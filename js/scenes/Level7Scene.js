var Level7Scene = new Phaser.Class({
  Extends: Phaser.Scene,
  initialize:

    function Level7Scene() { Phaser.Scene.call(this, { key: 'level7scene' }); },
  ...
  create: function () {
    ...
    this.level7_music = this.sound.add("level5_theme");
    var musicConfig = {
      mute: 0,
      volume: 0.5,
      seek: 0,
    ...
    this.level7_music.play(musicConfig);
    ...
    this.player = this.physics.add.sprite(1920 / 2 , 1080 / 2 - 400, 'player').setCollideWorldBounds(true);;
    ...
    this.time.addEvent({
      ...
      callback: function () {
        ...
        if (Phaser.Math.Between(0, 1000) >= spawnRate) {
          ...
          enemy8 = new Enemy8(
            this,
            this.game.config.width + Phaser.Math.Between(400, 600),
        ...
        if (enemy8 !== null) this.enemies.add(enemy8);
        if (enemy9 !== null) this.enemies.add(enemy9);
        if (diver3 !== null) this.divers.add(diver3);
      },
      callbackScope: this,
      loop: true
    });
  ...
  update: function () {
    ...
    if (cursors.left.isDown || this.keyA.isDown) this.player.setVelocityX(-500);
    else if (cursors.right.isDown || this.keyD.isDown) this.player.setVelocityX(600);
  ...
  playerHitsEnemy(player, enemy) {
    ...
    if (this.CollisionTimer === 0) {
      ...
      else this.player.setPosition(1920 / 2 , 1080 / 2 - 400);
    }
  },
  ...
  YouLost() {
    if (!this.FadeActive) {
      ...
      this.physics.pause();
      this.level7_music.stop();
      this.player.setTint(0xff0000);
      this.cameras.main.fadeOut(500, 0, 0, 0);
      this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, (cam, effect) => {
        this.scene.start('gameoverscene', { currentScene: 'level7scene'});
      });
    }
  },
  ...
  YouWon() {
    if (!this.FadeActive) {
      ...
      this.cameras.main.fadeOut(500, 0, 0, 0);
      this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, (cam, effect) => {
        this.scene.start('victoryscene', { nextScene: 'level8scene' });
      });
    }
  },
