var Level8Scene = new Phaser.Class({
  Extends: Phaser.Scene,
  initialize:

    function Level8Scene() { Phaser.Scene.call(this, { key: 'level8scene' }); },
  ...
  create: function () {
    ...
    this.level8_music = this.sound.add("level4_theme");
      var musicConfig = {
        mute: 0,
        volume: 0.4,
        seek: 0,
      ...
      this.level8_music.play(musicConfig);
    ...
    this.time.addEvent({
      ...
      callback: function () {
        ...
        if (Phaser.Math.Between(0, 1000) >= spawnRate) {
          if (Phaser.Math.Between(0, 1000) >= 750) {
            enemy4 = new Enemy4(
              this,
          ...
          else if (Phaser.Math.Between(0, 1000) >= 500) {
            enemy5 = new Enemy5(
              this,
              Phaser.Math.Between(-600, -400),
              Phaser.Math.Between(580, this.game.config.height - 580)
          ...
          else if (Phaser.Math.Between(0, 1000) >= 250) {
            enemy6 = new Enemy6(
              this,
              Phaser.Math.Between(-600, -400),
              Phaser.Math.Between(380, this.game.config.height - 380)
            );
          }
          else {
            enemy7 = new Enemy7(
        ...
        if (enemy4 !== null) this.enemies.add(enemy4);
        if (enemy5 !== null) this.enemies.add(enemy5);
        if (enemy6 !== null) this.enemies.add(enemy6);
        if (enemy7 !== null) this.enemies.add(enemy7);
    ...
    this.add.text(32, 16, 'Level: 8', { fontSize: '72px', fill: '#ffffff', fontFamily: 'pixel_font' })
  ...
  YouLost() {
    if (!this.FadeActive) {
      ...
      this.physics.pause();
      this.level8_music.stop();
      this.player.setTint(0xff0000);
      this.cameras.main.fadeOut(500, 0, 0, 0);
      this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, (cam, effect) => {
        this.scene.start('gameoverscene', { currentScene: 'level8scene' });
      });
    }
  },
  ...
  YouWon() {
    if (!this.FadeActive) {
      ...
      this.cameras.main.fadeOut(500, 0, 0, 0);
      this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, (cam, effect) => {
        this.scene.start('victoryscene', { nextScene: 'level9scene' });
      });
    }
  },
