var Level9Scene = new Phaser.Class({
  Extends: Phaser.Scene,
  initialize:

    function Level9Scene() { Phaser.Scene.call(this, { key: 'level9scene' }); },
  ...
  create: function () {
    ...
    this.level9_music = this.sound.add("level4_theme");
      var musicConfig = {
        mute: 0,
        volume: 0.4,
        seek: 0,
      ...
      this.level9_music.play(musicConfig);
    ...
    this.player = this.physics.add.sprite(1920 / 2 , 1080 / 2 , 'player').setCollideWorldBounds(true);;
    ...
    this.time.addEvent({
      ...
      callback: function () {
        ...
        if (enemy12 !== null) this.enemies.add(enemy12);
        if (enemy13 !== null) this.enemies.add(enemy13);
      },
      callbackScope: this,
      loop: true
    });
    ...
    this.time.addEvent({
      ...
      callback: function () {
        var enemy14 = null;
        var enemy14PossibleYPosition = [260, 540, 780]
        var enemy14YPosition =  enemy14PossibleYPosition[Math.floor(Math.random() * enemy14PossibleYPosition.length)];
        ...
        if (enemy14 !== null) this.enemies.add(enemy14);
      },
      callbackScope: this,
      loop: true
    });
    ...
    this.time.addEvent({
      ...
      callback: function () {
        var enemy15 = null;
        var enemy15PossibleYPosition = [375, 635]
        var enemy15YPosition =  enemy15PossibleYPosition[Math.floor(Math.random() * enemy15PossibleYPosition.length)];
        ...
        if (enemy15 !== null) this.enemies.add(enemy15);
      },
      callbackScope: this,
      loop: true
    });
    ...
    this.time.addEvent({
      ...
      callback: function () {
        ...
        var diver4 = null;
        var spawnRate = null;
        var diver4PossibleYPosition = [260, 540, 780]
        var diver4YPosition =  diver4PossibleYPosition[Math.floor(Math.random() * diver4PossibleYPosition.length)];

        if (this.Oxygen > 5000 && this.Oxygen <= 6000) {
          spawnRate = 0;
        }
        else if (this.Oxygen > 4000 && this.Oxygen <= 5000) {
          spawnRate = 60;
        }
        ...
        if (Phaser.Math.Between(0, 1000) >= spawnRate) {
          ...
          enemy16 = new Enemy16(
            ...
            enemy17 = new Enemy17(
              this,
              this.game.config.width + Phaser.Math.Between(500, 1000),
              Phaser.Math.Between(580, this.game.config.height - 580)
            );
          }
        }
        ...
        if (enemy16 !== null) this.enemies.add(enemy16);
        if (enemy17 !== null) this.enemies.add(enemy17);
        if (diver4 !== null) this.divers.add(diver4);
      },
      callbackScope: this,
      loop: true
    });
    ...
    this.add.text(32, 16, 'Level: 9', { fontSize: '72px', fill: '#ffffff', fontFamily: 'pixel_font' })
  ...
  playerHitsEnemy(player, enemy) {
    ...
    if (this.CollisionTimer === 0) {
      ...
      else this.player.setPosition(1920 / 2 , 1080 / 2 );
    }
  },
  ...
  YouLost() {
    if (!this.FadeActive) {
      ...
      this.physics.pause();
      this.level9_music.stop();
      this.player.setTint(0xff0000);
      this.cameras.main.fadeOut(500, 0, 0, 0);
      this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, (cam, effect) => {
        this.scene.start('gameoverscene', { currentScene: 'level9scene' });
      });
    }
  },
  ...
  YouWon() {
    if (!this.FadeActive) {
      ...
      this.cameras.main.fadeOut(500, 0, 0, 0);
      this.cameras.main.once(Phaser.Cameras.Scene2D.Events.FADE_OUT_COMPLETE, (cam, effect) => {
        this.scene.start('victoryscene', { nextScene: 'level10scene' });
      });
    }
  },
